# Lex’it – Lexicon Interactive Tool

[![License](https://img.shields.io/badge/license-Apache%202.0-blue.svg)](./LICENSE)  

---

## ✨ What is Lex’it?

**Lex’it** is a customizable web interface for databases.  

Out of the box, it allows you to:  
- View or edit database tables in a webpage  
- Search, page, and sort data with no extra configuration  
- Extend functionality via an extensive API  

Since Lex’it is built with **JavaScript** and **jQuery**, you can also integrate third-party JavaScript plugins to tailor projects to your needs.  

---

## 🚀 Quick Start

Clone the repository:  

```bash
git clone https://github.com/instituutnederlandsetaal/lexit
```

Go to the [`documents`](./documents) folder to find the manual:  
📄 **Lex'it_HowTo.pdf** – covers installation, setup, and development.  

---

## 📖 Documentation

- Full manual: [`documents/Lex'it_HowTo.pdf`](./documents/Lex'it_HowTo.pdf)  
- API reference: [`jsdoc/index.html`](./jsdoc/index.html)

## 🧑‍🔧 Maintainer

Principal Engineer: [Mathieu Fannee](mailto:mathieu.fannee@ivdnt.org)  

---

## 📜 License

This project is licensed under the **[Apache License 2.0](./LICENSE)**.  
