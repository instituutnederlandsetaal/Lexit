...
head.showNameOfTheTable = function(sSomeTablename){
	...
	// add notes or comments to a table (except if the table was uploaded as CSV/XLSX file)
	//

	if ( !fn.tableIsUploaded(sSomeTablename) ){

		tableNameElement.click(function(){
...
head.putHelpButton = function(sSomeTablename){
	...
	var helpButton = $("<button/>")
		...
		.bind("click", function(){
			...
			var bQueryBuilder = 	conf.queryBuilderAllowed(sSomeTablename);
			...
			$("#"+helpDivId).dialog({
				open: function( event, ui ){
					...
					// hide 'Query Builder' info if it is not allowed at all
					if (!bQueryBuilder){
						$(".query_builder_info").css("display", "none");
					}
		        },
...
head.putTableCloseButton = function(sSomeTablename){
	...
	var tableCloseButton = $("<button/>")
		...
		.bind("click", function(){
			...
			// remove table from menu as well
			//$("div#tablechoice select#selected_source").find("option[value='" + sSomeTablename + "']").remove();
		});
...
// put table-erase button
// (if allowed, t.i. when the table is an uploaded table)

head.putTableEraseButton = function(sSomeTablename){

	// if the table is NOT uploaded, leave right away
	if ( !fn.tableIsUploaded(sSomeTablename) ) return;
			...
			fn.confirm(lang.beware, lang.import_erase_confirm,
				function(){
					...
					$.ajax( {
						...
						"dataType": "xml", // get response as xml
						"success": function(xml) {
							fn.message( lang.succeeded+"!", lang.import_erase_table_done, function(){
						...
						"error": function(jqXHR, textStatus, errorThrown){
							...
							fn.message(lang.failed, lang.import_erase_table_failed+ ": "+textStatus+" "+errorThrown);
						}
					} );
				});
		});
