/**
 * The mt (=multitable) namespace is about table properties registration: 
 * It has no internal logic (which is organized in the other namespaces).
 * The only purpose of the mt namespace is to
...
// this consists of [0:description, 1:type (table/view), 2:comment ]
...
/**
 * Get the Datatables object of a table
 * @param {String} sSomeTablename - Table name
 * @returns {Object} A Datatables API instance
 */
...
mt.rememberLastSearchBoxClickUpon = function(sTableName, iIndex){
	...
	sSearchBoxThatHasFocus = mt.getListOfVisibleColumnsOf(sTableName)[iIndex];
...
mt.putUndoStacks = function(sSomeTableName, undoStack){
	undoStacks.put(sSomeTableName, undoStack);
};
mt.putUndoTooltips = function(sSomeTableName, tooltipStack){
	undoTooltips.put(sSomeTableName, tooltipStack);
};

mt.getUndoStacks = function(sSomeTableName){
	return undoStacks.get(sSomeTableName);
};
mt.getUndoTooltips = function(sSomeTableName){
