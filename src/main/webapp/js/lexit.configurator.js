...
var oTableSettingsList_example = {
		 ...
		 * if it needs to differ from the original database order. 
		 * (default: null)
		 * */
		...
		/** 
		 * @type {object} 
		 * @description Specifies the columns to be sorted, in order of priority, 
		 * provided as an associative array: { colname1: sortdir1, colname2: sortdir2, ...}
		 * */
		 ...
		 * (because multiple rows share the same value in that column), 
		 * it is possible to make these groups visible. 
		 * Use the "grouping_column" parameter to specify the column name for grouping. 
		 * 
		 * Note: This parameter only makes groups visually distinguishable on the screen. 
		 * To actually group the data, the table must be pre-sorted by the same column.      
		 */
		"grouping_column": "...",
		...
		/**
		 * @type {string}
		 * @description Height of the header above the table. 
		 * (default is 50px)
		 */
		"header_height": "150px",
		...
		/**
		 * @type {boolean}
		 * @description Determine whether the search box for the entire table 
		...
		/**
		 * @type {function} 
		 * @description Callback to be executed when clicking the reset button, 
		 * but just before the table is reloaded. 
		 * This allows modifications such as adjusting filters, etc.
		 */
		...
		/** 
		 * @type {boolean} 
		 * @description Refresh the table when the window in which it is displayed regains focus. 
		 * (default: true)
		 * */
		...
		/** 
		 * @type {string}
		 * @description When a 'menu' is declared, the "selected" option allows 
		 * preselecting a menu item. 
		 * The value should be the name of the menu option that should be preselected.
		 * */
		"selected": "...",
...
var oTableConfigurationList_example = {
		...
		/**
		 * @type {string}
		 * @description Background color of the column as an HTML color code (e.g., "#123ABC"). 
		 * If only one color code is provided, Lex'it automatically calculates a different 
		 * shade of that color for the odd rows. 
		 * 
		 * However, this does not work with literal color names (e.g., "green"). 
		 * It is also possible to specify the colors for both odd and even rows explicitly 
		 * by providing a two-element array with the two colors in sequence: 
		 * ["#123ABC", "#ABC123"].
		 */
		"bgcolor": "",
		 ...
		 * It is also possible to use the content of the column as the button name: 
		 * for this, an empty string should be provided for the name ("button": ""). 
		 * The functionality of the button must be specified using the 'click' parameter.
		 */
		"button": "",
		 ...
		 * This is useful for explaining the function of a button if the button's name 
		 * is insufficient (e.g., due to a short name).
		 */
		"button_tooltip": "",
		...
		/**
		 * @type {array}
		 * @description Turn the search box of the specified column into a select box. The values to choose from can 
		 * be specified in two ways: 
		 * [1] As an empty array, in which case the tool will automatically retrieve the unique values from the database 
		 * (e.g., {"choosefrom": []}). 
		 * [2] As an array of string values. Since the select box will have "Choose" as its first value, 
		 * "Choose" must also have an underlying value. This 'neutral' value should be the first element in the array, 
		 * and if no value needs to be assigned, an empty string will suffice (e.g., {"choosefrom": ["", "1", "2", "3"]}, 
		 * where "" will be the underlying value for "Choose").
		 */
		 ...
		 * This allows the incomprehensible values expected by the database 
		...
		/**
		 * @type {string}
		 * @description Assign a class to a column, for example, to style the cells of the column. 
		 * This could include default classes declared in css/lexit_table.css, such as those used for centering content in cells.
		 * 
		 * Lex'it provides some default classes, such as 'inlfont10pt' (for displaying the INL font in 10 pt) 
		 * or 'inlfont' (this one without a font-size specification). 
		 */
		"class": "",
		 ...
		 * @type {string}
		 * @deprecated
		 * @description If this is not null, the table will be immediately sorted by this column upon initialization. 
		 * If multiple columns are specified, the table will be sorted by all these columns. 
		 * The possible values are 'asc', 'desc', or null (default).
		 * 
		 * Note: If sorting by multiple columns at once, the priority is determined by the order in which the columns 
		 * to be sorted are specified. It is better and more organized to specify the sorting columns in the table settings 
		 * using the "columns_sorting" parameter (see there).  
		 */
		"colsort": "",
		 ...
		 * For more information about the context menu object, see the documentation, 
		...
		/**
		 * @type {function}
		 * @description Callback function to be called after a user edits a cell. 
		 * Note: This is different from 'editfunc'. 'editfunc' is used to modify the behavior of the edit function, 
		 * while 'editcallback' is executed last, after 'editfunc'.
		 */
		 ...
		 * This function is intended to replace the default handling of cell input: 
		 * normally, the software would update the edited cell directly in the database, 
		 * but when the screen table is a view, other tables need to be updated instead, 
		 * and this can be declared in 'editfunc'.
		 */
		...
		/**
		 * @type {boolean}
		 * @description Ellipsis. When a cell contains a lot of text, ellipsis can be enabled: only a portion of the text will be displayed. 
		 * On mouseover, the full content of the cell will be shown.
		 */
		"ellipsis": false,

		/**
		 * @type {integer}
		 * @description Ellipsis. When ellipsis is applied and the user moves the mouse over the table, the cells open one after another, 
		 * causing rows to swing open and closed, which can be very distracting. This also makes it difficult to select a row, 
		 * as nearby rows keep opening and closing. One solution is to set a delay, so that the expansion of a row only happens 
		 * if the mouse is still hovering over the cell after a specified duration. If not, the cell will not expand.
		 */
		"ellipsis_delay": 300,
		...
		/**
		 * @type {boolean}
		 * @description Ellipsis: Should a selected row remain unwrapped instead of automatically wrapping on mouseout? 
		 * (default: true)
		 */
		...
		/**
		 * @type {boolean}
		 * @description If this is 'false', a user cannot change the visibility of a column 
		 * (e.g., useful when a hidden column contains information not intended for the user).
		 */
		 ...
		 * 'RESET' button is clicked. If you want to prevent the filter from being modified in the meantime, 
		...
		/**
		 * @type {boolean}
		 * @description If this is 'false', the query builder will be disabled by CTRL+click in the search field.
		 */
		"query_builder_allowed": false,
		
		/**
		 * @type {object}
		 * @description Declare the search values (keys) to be shown in the query builder, 
		 * and their translation into database-understandable values (values) to construct a well-formed query.
		 * @example {"Neuter": "n", "Masculine": "m", "Feminine": "f"}
		 */
		"query_builder_values": {"key": "value"},
		
		/**
		 * @type {function}
		 * @description Function to modify the query formulated by the query builder before it is sent to the database.
		 */		
		"query_builder_processor": function(query){}, 
		
		/**
		 * @type {object}
		 * @description Settings for the query builder (if the default behavior needs to be adjusted). 
		 * Parameter 'grid': true/false determines whether the search values should be displayed as a grid.
		 * Parameter 'preselected' specifies which value should be preselected in the query builder.
		 * Parameter 'autostart': true/false determines whether the query formulated by the query builder 
		 * should be immediately sent to the database when clicking OK.
		 */
		...
		/**
		 * @type {function}
		 * @description Function to be triggered when a search is applied to a given column. 
		 * Example: automatically convert the search value to lowercase (this becomes the effective search value).
		 * @see searchform
		 * @see editpreprocess
		 */
		...
		/**
		 * @type {string}
		 * @description Color that the text should have in a given column.
		 */
		"textcolor": "",
		
		/**
		 * @type {string}
		 * @description Font-family for a given column.
		 */
		"textfont": "",
		
		/**
		 * @type {string}
		 * @description Font-size for a given column.
		 */
		"textsize": "",
		
		/**
		 * @type {string}
		 * @description Font-style for a given column.
		 */
		"textstyle": "",
		
		/**
		 * @type {string}
		 * @description Font-weight for a given column. See the example at 'textstyle'. 
		 */
		"textweight": "",
		 ...
		 * To prevent this, you can enforce in the configuration that the user must press a specified key 
		 * while making their selection. If the user does not press this key when making a selection in the select box, 
		 * the choice will be rejected. This validation key can be specified as 'validator'.
		 */
		"validator": "",
		
		/**
		 * @type {boolean}
		 * @description Visibility of a column. The visibility of a column can be changed by the user in the interface. 
		...
		/**
		 * @type {string}
		 * @description Width of a column. This can be specified in 'px', 
		 * but a relative value (in '%') can also be provided.
		 */
		"width": "10%"
...
conf.getQueryBuilderAllowed = function(oColumnConfig){
	if (typeof oColumnConfig["query_builder_allowed"] == 'undefined')
	    return true;
	return oColumnConfig["query_builder_allowed"];
};
...
// is the query build allowed at all in this table?
// (one column where it is allowed is enough to allow the whole table)
conf.queryBuilderAllowed = function(sSomeTablename){
	
	var oTableConfig = conf.getTableConfig(sSomeTablename);
	
	var aRegexColumnConfig = conf.getColumnConfig(oTableConfig, "*");
	
	if (conf.getQueryBuilderAllowed(aRegexColumnConfig) == false)
	    return false;
	
	var bAllowed = false;
	var aCols = mt.getListOfVisibleColumnsOf(sSomeTablename);
	for (var oneColumns in aCols) {
		var aColumnConfig = conf.getColumnConfig(oTableConfig, oneColumns);
		if (conf.getQueryBuilderAllowed(aColumnConfig)){
			bAllowed = true;
			break;
		}
	}
