...
// storage of forms lists

var hFormToLists = new Hashtable();
...
form.buildViewGrid = function(sTableName){
	...
	// if we don't have a lists object yet for the form,
	// instantiate the lists object now

	var lists = hFormToLists.get(sTableName);

	if (lists == null){
		lists = new Lists(sTableName);
		hFormToLists.put(sTableName, lists);
	} 
	...
	for (var sListLabel in oLists){
		...
		var listContainer = $("<div></div>")
			...
			.css("background-color", sBgColor)
			.attr("id", lists.buildTableContainerId(sListLabel));
		...
		lists.register(sListLabel, sTableFeedingList, (parseFloat(aListDefinition[1]) *iGridHeightUnit) +"px", oSortSettings);
	}
