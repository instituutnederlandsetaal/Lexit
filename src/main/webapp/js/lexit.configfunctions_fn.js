...
fn.getLibrary = function(sPath, fnCallback, fnErrorHandler){
	...
	// in test mode, we load the file locally, but on the server we load the file from the 'lexit2_config' folder 
...
/**
 * Load multiple libraries with extra functions for a given project
 * 
 * @param {String} sPathToFolder - Path to the folder where the library files are located
 * @param {String[]} aLibraryFiles - Array with the names of the library files to be loaded (eg. ['library1.js', 'library2.js'])
 * @param {Function} [fnCallback=null] - Function to be called after loading the libraries
 * @param {Function} [fnErrorHandler=null] - Some function to call when an error occurs
 * @see fn.getLibrary
 */
fn.getLibraries = function(sPathToFolder, aLibraryFiles, fnCallback, fnErrorHandler){
	...
	// of course, if a full path was declared, we should leave it as is
	if (!$.startsWith(sPathToFolder, "http") && !$.startsWith(sPathToFolder, "..")){
		var sPrefix = (paramsHash.get("test")=='true') ? "" : "../lexit2_config/";
		// create a path consisting of the prefix and the library subfolder attached  
		sPathToFolder = sPrefix + sPathToFolder;	
	}
	
	// If the path lacks a separator as the end, add it
	var sSeparator = "";
	if ( sPathToFolder != '' && !$.endsWith(sPathToFolder, "/") && !$.endsWith(sPathToFolder, "\\") ){
		sSeparator = "/";
	}
	sPathToFolder = sPathToFolder + sSeparator;
	
	// start loading the libraries, one by one
	fn._getLibraries(sPathToFolder, aLibraryFiles, fnCallback, fnErrorHandler);
}

// subfunction to load multiple libraries with extra functions for a given project
fn._getLibraries = function(sPathToFolder, aLibraryFiles, fnCallback, fnErrorHandler, iCurrentIndex=0){
	
	// if loading is done, call the callback and exit
	if (iCurrentIndex >= aLibraryFiles.length) {
		if (fnCallback != null)
			fnCallback();
		return;
	}
	// otherwise load the current library, 
	// and when done, call this function again for the next library 
	else {
		
		// effectively load the library
		fn.getLibrary(sPathToFolder + aLibraryFiles[iCurrentIndex], function(){
			
			// increase the index for the next round
			iCurrentIndex++;
			fn._getLibraries(sPathToFolder, aLibraryFiles, fnCallback, fnErrorHandler, iCurrentIndex);
		});
...
fn.prompt = function(sTitle, aFieldNames, aValues, fnFunction, fnCancelFunction, bTextarea, aColsAndRows){
	...
	for (var i=0; i<aFieldNames.length; i++) {
		...
		if (bSelectBox) {
			...
			var bBuildFaster = false;
			
			if (bBuildFaster) {
				
				// this is faster code generated by ChatGPT
				
				var input = $("<select></select>")
				    .attr("id", "prompt_" + fieldLC)
				    .prop('disabled', bFixedValue);
				
				// Use a string to batch all option elements
				var optionsHTML = "";
				...
				for (var j = 0; j < aValues[i].length; j++) {
				    ...
				    // Build each option as a string and append to optionsHTML
				    optionsHTML += `<option value="${sThisValue}" ${bSelected ? 'selected' : ''}>${sThisValue}</option>`;
				}
				
				// Append all options at once as an HTML string
