...
asr.alterTable = function(sSomeTablename, addOrReplaceMode, bReallyChange){
	...
	$("#"+sSomeTablename+" tbody tr").each(function(){
		for (var i=0; i<filterColumnList.length; i++)
			...
			var sCell;
			var oRowData = mt.getDataTableObjectOf(sSomeTablename).row(this).data();
			
			if (typeof rowData == 'object')
				sCell = oRowData[sColumnName];
			else
				sCell = oRowData[trueIndexOfColumn];
	...
	aRowsToProcess.each(function(){
		...
		for (var i=0; i<filterColumnList.length; i++)
			...
			if (typeof rowData == 'object')
				sCell = oRowData[filterColumnList[i]];
			else
				sCell = oRowData[indexOfColumn];
	...
	if (bReallyChange)
		...
		if (addOrReplaceMode=="replace")
			...
			// update the database
			var url = WEBSERV_URL+"/table/setvalue_without_id_for_search_and_replace"; 
			$.ajax( {
				"type": "GET",
				"url": url,
				"data": {
			...
			var url = WEBSERV_URL+"/table/insertmodified_without_id";
