		...
		lemmata_and_wordforms : {
			...
			"callback": function(confTable){
				...
				allRows.each(function(){
					...
					if (id!='')
						fn.callFunction("get_awf_parts", [id], confTable, this, "parts");					
					...
					fn.callDatabase("lemmata_and_wordforms", {"lemma_id": lemmaId}, 
							function(){
								...
								if (!fn.tableExists("lemmata_selectie_view")) 
									fn.callDatabase("lemmata_selectie_view", {}, function(){
										fn.alignTables("lemmata_and_wordforms", "lemmata_selectie_view");
									});
							}
					);			
		...
		lemmata_and_wordforms : {
			...
			wordform: {
				...
				"click": function(confTable, confNode){					
					...
					fn.callFunction("get_wordforms", [sWordformId], 
						...
						if (aWordforms.length > 1)
							...
							fn.promptReorder("Volgorde aanpassen", aWordforms, 
								...
								fn.callFunction("update_wordform_set_wordnumbers", [aWordformsInNewOrder.join("|")], 
										null, null, null, null, function(){

										fn.callFunction("concatenate_one_wordform", [awfId], 
		...
		token_attestations_view : {
					...
					if (oSelectedText.text!='' && oSelectedText.reliable)
						...
						// process the selected token
							...
							fn.callFunction("update_tokens", 
									[sTokenIndexesId, iStart, iEnd, oSelectedText.text], 
									null, null, null, null, 
									function(){
			...
			add_attestation : {
				...
				"click": function(confTable, confNode){
					...
					fn.callFunction("clone_attestation_record", 
							[ sQuote, iAwfId, "-1", iDocId ], 
			...
			form_pos : {
				...
				"click": function(confTable, confNode){
					...
					fn.callFunction("get_token_list", [sTokenIndexId], 
							null, null, null, null, 
							function(){
						...
						// we have more tokens
							...
							{
							// in that case we need to update the awf_parts table
							fn.prompt("Vormen en woordsoorten", fn.getFunctionOutput(), null,
								...
								fn.callFunction("reorder_analyzed_wordform_parts", 
										[sListOfTokens, 
										 slistOfPos, 
										 sAwfId,
										 sAttestationId], 
...
function putPartformsAndPos(confTable){
	...
	aAllRows.each(function(){
		...
		fn.callFunction("get_parts_and_pos", [awfId], 
...
function putPartsAndTokensEquality(confTable){
	...
	aAllRows.each(function(){
		...
		fn.callFunction("get_parts_and_tokens_equality", [iTokenIndexesId], 
