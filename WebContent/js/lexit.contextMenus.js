	...
	// build content menu
	 $.contextMenu({
	        ...
	        callback: function(key, options){
	        	...
	        	var confTable = mt.getDataTableObjectOf(sSomeTable);
	        	
	        	var oTrParentOfTd = options.$trigger.parent();
	        	var aAllTrs = oTrParentOfTd.parent().children();
	            var yPos = aAllTrs.index(oTrParentOfTd);	
	            var nNode = confTable.fnGetNodes(yPos);
	            // if no column name is set, use row nodes
	            var confNode = sColumnName != null ?
	            		fn.getCellNode(sSomeTable, nNode, sColumnName) : nNode;	            
	            
	            // call the user callback with the right table/node and context menu keys/options info
	        	fnCallback(confTable, confNode, key, options);
	        },
	        items: oItems
	    }); 
};
