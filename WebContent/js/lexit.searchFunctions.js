...
sf.goTo = function(sSomeTablename){
	...
	if (sColumnName=="")
		{
		alert("Tik een zoekterm in een zoekbox boven een kolom, en klik dan pas op 'Ga naar'!");
		return true;
		}
...
sf.getRowNumber = function(sSomeTablename, sColumnName, sColumnValue, sSortColumn, sSortDirection, filterColumnNames, filterValues){
	...
	if (sSortColumn == null || sSortColumn == "")
		{
		alert("De tabel is niet gesorteerd op een kolom. De functie 'Ga naar' werkt niet zonder sortering. Sorteer eerst de tabel op een kolom.");
		return;
		}
	
	gui.showProcessingMsg(sSomeTablename);
	
	var url = "/lexit/lexit/table/get_row_number";
	...
	$.ajax(
			{
				...
				error: function(jqXHR, textStatus, errorThrown){
					gui.removeProcessingMsg(sSomeTablename);
					alert("XML laden mislukt: "+textStatus+" "+errorThrown);
					}
			});
};
	...
	$('#'+someTablename+' thead th').each( function(i){
		...
		if (isACheckBox)
			...
			$("#"+someTablename+"_dynamic").on('click', "#"+someTablename+"_searchboxes div:eq("+i+")", function(){				
				...
				mt.getDataTableObjectOf(someTablename).fnFilter( sCorrectCheckboxFilterValue, iTrueIndex );
		...
		// add change event for selection box filters
			...
			$("#"+someTablename+"_dynamic").on('change', "#"+someTablename+"_searchboxes div:eq("+i+") select", function(){
				...
				// fire filter straight away
				mt.getDataTableObjectOf(someTablename).fnFilter( $(this).val(), iTrueIndex);
...
sf.startSearch = function(someTablename){
	...
	mt.getDataTableObjectOf(someTablename).fnGlobalFilterSet(searchBoxValue);
	
	// start search
	mt.getDataTableObjectOf(someTablename).fnDraw();
};
...
sf.startMultiColumnSearch = function(someTablename){
	...
	$("#"+someTablename+"_searchboxes div").each(function(i){
		...
		if (!isACheckBox && jQuery.trim(oneSearchBoxValue)!='')
			...
			mt.getDataTableObjectOf(someTablename).fnSetOneFilter( oneSearchBoxValue, iTrueIndex );
			}
		...
		if (isACheckBox)
			...
			if (oneSearchBoxValue != "")
				mt.getDataTableObjectOf(someTablename).fnSetOneFilter( ( oneSearchBoxValue=="1" ? trueValue : falseValue ), iTrueIndex);
			// if the checkbox has no value, empty the filter
			else
				mt.getDataTableObjectOf(someTablename).fnSetOneFilter( "", iTrueIndex);
			}
