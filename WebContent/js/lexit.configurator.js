/**
 * This class takes care of reading the client configuration
 */
...
conf.changeTableConfigValue = function(sSomeTableName, sColumnName, sSettingName, value){
	...
	if (oTableConfig == null) oTableConfig = new Object();		
...
// retrieve values to choose from, if we need the column filter to be a selection box
// default is null
		...
		if (typeof aColumnConfig[sOneMouseActionType] != 'undefined')
			...
			$('#'+sSomeTablename+'_dynamic').on(sMouseEvent, '#'+sSomeTablename+' tbody td.'+iColumnNr,
					...
					function(){			
						...
						if (fn.getNumberOfSelectedRows(sSomeTablename)==null)
							{
							...
							if ( lastSelectedRowForThisTable != null )
								...
								if ( !bPreviousStateWasSelected )
									{
									lastSelectedRowForThisTable.removeClass('row_selected');									
									}
								}
							// now select the current row
							var bRowWasAlreadySelectedBeforeSelection = $(this).parent().hasClass('row_selected');
...
conf.activeContextMenusForColumns = function(sSomeTablename){
...
conf.activeContextMenusForRows = function(sSomeTablename){
...
conf.getHeaderButtonNr = function(aTableSettings, iButtonNumber){
